...
-(void)panHandle:(UIPanGestureRecognizer *)pan{
    ...
    if (pan.state == UIGestureRecognizerStateBegan) {
        ...
        [cardView setImage:[UIImage imageNamed:[NSString stringWithFormat:@"card%zd",self.num % 4]]];
        cardView.tag = self.num % 4;
        
        cardView.frame = CGRectMake(0, 0, 300, 200);
        [cardView config];
        cardView.hidden = YES;
        ...
        [self setCenter:CGPointMake(CGRectGetWidth(self.view.bounds)/2, CGRectGetHeight(self.view.bounds)/2+0-20-(sizePercent*0*200)) Duration:0.25 Card:cardView Index:0];
    ...
    for (int i =1; i<self.cards.count-1; i++) {
        ...
        CGPoint subCardCenter = CGPointMake(CGRectGetWidth(self.view.bounds)/2, CGRectGetHeight(self.view.bounds)/2+0-20-sizePercent*200*(i-1+(fabs(percent))));
...
-(void)showCardZone{
    ...
    for (int i = 0; i<4; i++) {
        ...
        [card setImage:[UIImage imageNamed:[NSString stringWithFormat:@"card%d",3 - i]]];
        card.tag = 3 - i;
        
        card.frame = CGRectMake(0, 0, 300, 200);
        [card config];
        [self setScaleWithScalePercent:(1-sizePercent*(3-i)) Duration:0.0001 Card:card];
        CGPoint targetCenter = CGPointMake(CGRectGetWidth(self.view.bounds)/2, CGRectGetHeight(self.view.bounds)/2+0-20-(sizePercent*i*200));
...
-(void)cardReCenterOrDismiss:(BOOL)isDismiss Card:(cardView *)card{
    if (isDismiss) {
        ...
        if (card.center.x<CGRectGetWidth(self.view.bounds)/2) {
            [self setCenter:CGPointMake(0-0, card.center.y) Duration:0.25f Card:card Index:0];
        }
        else
        {
    ...
    else
    {
        ...
        [self setRorationWithAngle:0 Duration:0.25 Card:card];
        CGPoint targetCenter = CGPointMake(CGRectGetWidth(self.view.bounds)/2, CGRectGetHeight(self.view.bounds)/2+0-20-(sizePercent*3*200));
